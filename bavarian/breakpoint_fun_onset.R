#' Function to estimate a piecewise-linear model quasipoisson model for the analysis
#' of new COVID-19 infections per day. Breakpoints are found based on discrete optimization
...
estimate_bp_disc_optim = function(data, bp=3) {
  ...
  create_data_est_model = function(bp_vec, data = dat) {
    ...
    mod = tryCatch(glm(backpro~., family = quasipoisson, data = dat_mod), error = NULL)
  ...
  # Re-estimate model
  mod = glm(backpro~., family = quasipoisson, data = dat_mod)
...
estimate_bp_segmented = function(data, bp = 3, start_bp = c(18,23,54), 
  ...
  dat = data %>% mutate(t=1:n())
  mod1 = glm(backpro~t, family=quasipoisson, data=dat)
  ...
  bp_plot = ggplot() +
    ...
    ggsave("./graphics/disease_onset_bav.png", width = 9, height = 5)
  
  
  
  # Plot: Number of infections
  # Zeitachse einfach um 5 Tage nach links verschoben  (e.g. Ende dann am 26.4. statt am 1.5)
  shift <- 5   # Verschiebung um 5
  dat$date <- dat$date - shift
  
  bp_plot_infection = ggplot() +
    geom_col(data = dat, aes(x=t, y=backpro),
             col = "grey", fill = "lightgrey") +
    geom_rect(aes(xmin=floor(break_points$bp_lwr) - 0.49,
                  xmax=ceiling(break_points$bp_upr) + 0.49, ymin=0, ymax=Inf), fill = "steelblue", alpha = 0.25)+
    ...
    geom_vline(xintercept=break_points$bp, lty=2, col ="steelblue", size=0.8) +
    theme_bw()+
    scale_x_continuous(breaks = seq(min(dat$t) + 9, max(dat$t) + shift, by = 7), # gleicher Starttag wie disease onset Graphik
                       labels = strftime(seq(min(dat$date) + 9, max(dat$date) + shift, by = "1 weeks"), 
                                         format = "%d.%m."))+
    scale_y_continuous(expand=expansion(mult = c(0, .1))) +
    # geom_text(mapping=aes(x=break_points$bp,  y = 20, label = strftime(break_points$bp_date, format = "%d.%m.")), 
    #           size = 5, angle = 90, vjust =-.5, hjust = -0.1) +
    ...
    theme(
      ...
      legend.title =element_text(size = rel(1.5)),
      legend.position = "bottom") +
    ggsave("./graphics/disease_onset_bav_shifted.png", width = 9, height = 5)
  list(segmented_model = segmented, coef=mod_res, breakpoints = bp_dates_res, plot = bp_plot, plot_infections = bp_plot_infection)
}
