...
estimate_bp_segmented = function(data, bp = 3, start_bp = c(18,23,54), 
  ...
  bp_plot = ggplot() +
    ...
    ylim(0, 6500) +
    ggsave("./graphics/disease_onset_ger.png", width = 9, height = 5)
  ...
  bp_plot_infection = ggplot() +
    ...
    geom_vline(xintercept=break_points$bp, lty=2, col ="steelblue", size=0.8) +
    theme_bw()+
    scale_x_continuous(breaks = seq(min(dat$t) + 7, max(dat$t) + shift, by = 7), 
                       labels = strftime(seq(min(dat$date) + 7, max(dat$date) + shift, by = "1 weeks"), 
                                         format = "%d.%m."))+
    scale_y_continuous(expand=expansion(mult = c(0, .1))) +
    # geom_text(mapping=aes(x=break_points$bp,  y = 20, label = strftime(break_points$bp_date, format = "%d.%m.")), 
    #           size = 5, angle = 90, vjust =-.5, hjust = -0.1) +
    ...
    theme(
      ...
      legend.position = "bottom") +
    ylim(0, 6500) +
    ggsave("./graphics/disease_onset_ger_shifted.png", width = 9, height = 5)
