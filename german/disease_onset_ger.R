...
covid_ts <- covid_ts %>% mutate(Est_Onsets = if_else(is.na(Est_Onsets.x), Est_Onsets.y, Est_Onsets.x))
...
# Estimate segmented regression with 3 or 4 Breakpoints based on 'segmented' package
# with starting points from discrete optimization
...
pdf("results/res-ger-disease-onset.pdf", width = 9, height = 6)
res_4_seg$plot
dev.off()
